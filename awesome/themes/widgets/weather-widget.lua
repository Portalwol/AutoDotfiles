...
local WIDGET_DIR = HOME_DIR .. '/.config/awesome/themes/widgets/'
local GET_FORECAST_CMD = [[bash -c "curl -s --show-error -X GET '%s'"]]

local SYS_LANG = os.getenv("LANG"):sub(1, 2)
-- default language is ENglish
local LANG = gears.filesystem.file_readable(WIDGET_DIR ..
                                      SYS_LANG .. ".lua") and SYS_LANG or "en"
local LCLE = require("themes.widgets.en")
...
local function worker(user_args)
    ...
    local font_name = "SF Pro Display"
    local units = args.units or 'metric'
    local time_format_12h = true
...
-- Dante Robinson changed ICONS_DIR

    local ICONS_DIR = HOME_DIR .. '/.config/awesome/themes/icons/'
